...
private:
  ...
  /// The Minecarft server port
  uint16_t server_port;
  /// A number of seconds at which the query will be cancelled.
  int timeout;
  /// The protocol version that will be sent to the server inside of the Handshake packet.
  ...
  /// the MC server.
  [[nodiscard]] std::string _query_slp() const;
...
public:
  ...
  /// Add the timeout.
  [[nodiscard]] std::string query_slp() const;
};
