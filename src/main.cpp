...
int main(const int argc, char *argv[]) {
    ...
    try {
        slp serv{SlpOptions{}, [](const Outcome &out) {
                     enable_output();
                     std::cout << out->json << std::endl;
                 }};
        // serv, timeout, protocolvers
        serv.submit({.server = {addr, port},
                     .timeout = std::chrono::milliseconds(std::chrono::seconds(static_cast<size_t>(timeout)))});
        serv.seal();
        serv.finish();
        sleep(22222);
        // Print JSON
